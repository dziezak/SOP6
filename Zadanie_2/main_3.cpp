...
void server(int n_workers) {
    ...
    struct mq_attr attr = {0, 10, sizeof(Task), 0}; // Kolejka na maks 10 zadan
    ...
    //kolejki wynikowe!
    mqd_t result_queues[n_workers];
    for(int i=0; i<n_workers; i++){
        char result_queue_name[256];
        sprintf(result_queue_name, RESULT_QUEUE_NAME, getpid(), i);

        struct mq_attr result_att = {0, 10, sizeof(float), 0};
        result_queues[i] = mq_open(result_queue_name, O_CREAT | O_RDONLY, 0644, &result_att);
        if(result_queues[i] == -1){
            perror("mq_open (server result)");
            exit(EXIT_FAILURE);
        }
    }
    ...
    while (received_results < total_tasks) {
        for (int i = 0; i < n_workers; i++) {
            float result;
            if (mq_receive(result_queues[i], (char*)&result, sizeof(float), NULL) > 0) {
    ...
    printf("Server: I'v done my work\n");
    while (wait(NULL) > 0);
...
void worker(int worker_id) {
    ...
    mqd_t result_queue = mq_open(result_queue_name, O_CREAT | O_WRONLY);
    if (result_queue == -1) {
